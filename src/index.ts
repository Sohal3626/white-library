type Book = { id: string; title: string; emotion: string; description: string; energy: number; risk: number; contained: boolean; shelfId?: number };
type EmotionName = "희망" | "불안" | "공포" | "슬픔" | "분노" | "애착";
type EmotionEnergy = Record<EmotionName, number>;
type CrisisState = { nextDay: number; eventId: string; warned: boolean };
type GameState = { day: number; health: number; food: number; firewood: number; energy: number; emotionEnergy: EmotionEnergy; warmth: number; houseLevel: number; shelves: number; shelfRooms: number[]; books: Book[]; discoveredBookIds: string[]; tools: string[]; crisis: CrisisState };
type EventKind = "none" | "avalanche" | "animal" | "resource" | "book" | "story";
type RouteKind = "safe" | "ruins" | "tracks" | "signal";
type RouteOption = { id: string; kind: RouteKind; actualKind: RouteKind; title: string; clue: string; tendency: string; risk: string };
type Expedition = { totalTurns: number; turn: number; hp: number; packedFood: number; packedWood: number; selectedBookIds: string[]; selectedToolIds: string[]; consumedToolIds: string[]; lootToolIds: string[]; lootFood: number; lootWood: number; lootBooks: Book[]; event: EventKind; animal?: "토끼" | "늑대" | "곰"; discovery?: Book; bookChoiceOrder?: number[]; storyId?: string; routes?: RouteOption[]; chosenRoute?: RouteOption; phase: "route" | "encounter" | "decision" | "result"; resultText: string };
type StoryChoice = { label: string; hint: string; result: string; food?: number; wood?: number; health?: number; chance?: number; failure?: string; failureHealth?: number; failureFood?: number; failureWood?: number; requiredTool?: string; consumeTool?: boolean; grantTool?: string };
type StoryEvent = { id: string; title: string; scene: string; choices: StoryChoice[] };
type Tool = { id: string; name: string; description: string; consumable: boolean; special?: boolean };
type SynergyTag = "황금" | "숲" | "가족" | "왕실" | "변신" | "약속" | "노동" | "죽음" | "동물" | "마법" | "기지" | "탐욕" | "구원" | "배신";
type ShelfSynergy = { tag: SynergyTag; shelfId: number; count: number; bonus: number; emotion: EmotionName };
...
const toolById = (id: string) => TOOLS.find(tool => tool.id === id);
const HIDDEN_BURN_REWARDS: Record<string, string> = {
...
const EXPEDITION_STORIES: StoryEvent[] = [
...
const TOOL_STORY_CHOICES: Partial<Record<string, StoryChoice[]>> = {
...
const ROUTE_VARIANTS: Record<RouteKind, Array<Omit<RouteOption, "id" | "kind" | "actualKind">>> = {
  safe:[
...
const ROUTE_STORY_POOLS: Record<RouteKind, string[]> = {
...
const BOOK_TAGS: Record<string, SynergyTag[]> = {
...
const SYNERGY_EMOTIONS: Record<SynergyTag, EmotionName> = { 황금:"분노", 숲:"불안", 가족:"애착", 왕실:"희망", 변신:"불안", 약속:"애착", 노동:"희망", 죽음:"슬픔", 동물:"공포", 마법:"공포", 기지:"희망", 탐욕:"분노", 구원:"희망", 배신:"불안" };
const EMOTIONS: EmotionName[] = ["희망", "불안", "공포", "슬픔", "분노", "애착"];
...
type BookPuzzle = { scene: string; choices: [string, string, string]; correct: number; success: string; failure: string };
const BOOK_PUZZLES: Record<string, BookPuzzle> = {
...
type CrisisEffect = { health?: number; food?: number; firewood?: number; warmth?: number; loseBook?: boolean };
type CrisisChoice = { label: string; description: string; cost?: Partial<EmotionEnergy>; result: string; effect?: CrisisEffect };
type MidgameCrisis = { id: string; title: string; emotions: EmotionName[]; omen: string; scene: string; choices: [CrisisChoice, CrisisChoice] };
const MIDGAME_CRISES: MidgameCrisis[] = [
...
function catalogBook(index: number): Book { const title = CATALOG_TITLES[index]; return { id: `khm-${index + 1}`, title, emotion: EMOTIONS[index % EMOTIONS.length], description: OBSERVATIONS[title] || `이 기록에는 ${title}라 불린 사건의 흔적이 남아 있다. 아직 모든 문장을 해독하지 못했지만, 등장인물들이 끝까지 놓지 못한 감정이 종이 깊숙이 얼어붙어 있다.`, energy: 1 + (index % 4), risk: 1 + (index % 3), contained: true }; }
const emptyEmotionEnergy = (): EmotionEnergy => ({ 희망: 0, 불안: 0, 공포: 0, 슬픔: 0, 분노: 0, 애착: 0 });
function createCrisisState(day = 15): CrisisState { return { nextDay: day, eventId: MIDGAME_CRISES[Math.floor(Math.random() * MIDGAME_CRISES.length)].id, warned: false }; }
const initialState = (): GameState => { const firstBook = catalogBook(0); firstBook.shelfId = 0; return { day: 1, health: 100, food: 8, firewood: 7, energy: 3, emotionEnergy: { ...emptyEmotionEnergy(), 희망: 3 }, warmth: 68, houseLevel: 1, shelves: 1, shelfRooms: [1], books: [firstBook], discoveredBookIds: [firstBook.id], tools: [], crisis: createCrisisState() }; };
...
let booksView: "shelves" | "all" = "shelves";
let selectedBookShelfId = 0;
let prepFood = 1;
let prepWood = 1;
let prepBookIds: string[] = [];
let prepToolIds: string[] = [];
let expedition: Expedition | null = null;
function saveGame() { if (state) localStorage.setItem(SAVE_KEY, JSON.stringify(state)); }
function loadGame(): GameState | null { const raw = localStorage.getItem(SAVE_KEY); if (!raw) return null; try { const saved = JSON.parse(raw) as GameState; if (!saved.books.some(book => book.id.startsWith("khm-"))) saved.books = [catalogBook(0)]; if (!saved.emotionEnergy) saved.emotionEnergy = { ...emptyEmotionEnergy(), 희망: saved.energy || 0 }; if (!saved.shelfRooms || saved.shelfRooms.length !== saved.shelves) saved.shelfRooms = Array.from({ length: saved.shelves }, (_, index) => 1 + Math.floor(index / 2)); saved.books.forEach((book, index) => { if (typeof book.shelfId !== "number" || book.shelfId >= saved.shelves) book.shelfId = Math.min(saved.shelves - 1, Math.floor(index / 4)); }); if (!Array.isArray(saved.tools)) saved.tools = []; if (typeof saved.health !== "number") saved.health = 100; if (!saved.crisis) saved.crisis = createCrisisState(Math.max(15, Math.ceil((saved.day + 1) / 15) * 15)); saved.energy = EMOTIONS.reduce((sum, emotion) => sum + (saved.emotionEnergy[emotion] || 0), 0); return saved; } catch { localStorage.removeItem(SAVE_KEY); return null; } }
function svg(name: "book" | "fire" | "food" | "home" | "snow") { const paths = { book: '<path d="M5 4.5A3.5 3.5 0 0 1 8.5 1H19v18H8.5A3.5 3.5 0 0 0 5 22.5z"/><path d="M5 4.5A3.5 3.5 0 0 0 1.5 1H1v18h.5A3.5 3.5 0 0 1 5 22.5"/>', fire: '<path d="M13 22c4-1 7-4 7-8 0-3-2-6-5-8 0 3-1 5-3 6 0-5-2-8-5-11 0 5-4 8-4 13 0 4 3 7 7 8-2-2-2-5 1-7 0 3 3 4 2 7z"/>', food: '<path d="M4 11h16c0 6-3 10-8 10s-8-4-8-10z"/><path d="M7 7c0-2 1-3 2-4m4 4c0-2 1-3 2-4"/>', home: '<path d="m2 11 10-9 10 9"/><path d="M5 9v12h14V9M9 21v-7h6v7"/>', snow: '<path d="M12 2v20M4 7l16 10M20 7 4 17M9 4l3 3 3-3M9 20l3-3 3 3"/>' }; return `<svg viewBox="0 0 24 24" aria-hidden="true">${paths[name]}</svg>`; }
function toast(message: string) { const el = document.createElement("div"); el.className = "toast"; el.textContent = message; document.body.append(el); window.setTimeout(() => el.remove(), 2200); }
function renderMenu() {
  ...
  app.querySelector('[data-action="new"]')?.addEventListener("click", () => { if (hasSave && !confirm("기존 기록을 덮어쓰고 새 게임을 시작할까요?")) return; state = initialState(); saveGame(); renderHouse(); });
  app.querySelector('[data-action="continue"]')?.addEventListener("click", () => { state = loadGame(); if (state) renderHouse(); });
  app.querySelector('[data-action="reset"]')?.addEventListener("click", () => { if (!confirm("저장된 모든 기록을 지울까요?")) return; localStorage.removeItem(SAVE_KEY); renderMenu(); });
}
...
function activeShelfSynergies(): ShelfSynergy[] {
  ...
  for (let shelfId = 0; shelfId < state.shelves; shelfId++) {
    ...
    counts.forEach((count, tag) => { if (count < 2) return; const bonus = count >= 4 ? 4 : count === 3 ? 2 : 1; const synergy = { tag, shelfId, count, bonus, emotion: SYNERGY_EMOTIONS[tag] }; const list = candidates.get(tag) || []; list.push(synergy); candidates.set(tag, list); });
  }
  return [...candidates.values()].map(entries => entries.sort((a, b) => b.count - a.count || a.shelfId - b.shelfId)[0]);
}

function synergiesForShelf(shelfId: number) { return activeShelfSynergies().filter(synergy => synergy.shelfId === shelfId); }
function shelfSynergyCandidates(shelfId: number): ShelfSynergy[] {
  if (!state) return [];
  const counts = new Map<SynergyTag, number>();
  state.books.filter(book => (book.shelfId ?? 0) === shelfId && bookIsPowered(book)).forEach(book => (BOOK_TAGS[book.title] || []).forEach(tag => counts.set(tag, (counts.get(tag) || 0) + 1)));
...
function emotionProduction(): EmotionEnergy {
  const production = emptyEmotionEnergy();
  if (!state) return production;
  state.books.filter(bookIsPowered).forEach(book => production[book.emotion as EmotionName] += book.energy);
...
function renderInteractiveHouse() {
  ...
  if (state.health <= 0) return renderGameOver("이 피난처의 마지막 생존 기록은 여기에서 끝났다.");
  if (state.crisis.nextDay <= 90 && state.day >= state.crisis.nextDay) return renderMidgameCrisis();
  if (state.crisis.nextDay <= 90 && state.day >= state.crisis.nextDay - 3 && !state.crisis.warned) return renderCrisisWarning();
  ...
  const roomShelfCounts = new Map<number, number>();
  ...
  app.innerHTML = `<main class="game-screen spatial-screen">
    ...
    <section class="house-viewport" aria-label="집 내부"><div class="drag-hint">↔ 드래그하여 둘러보기</div><div class="house-world" style="width:${worldWidth}px;transform:translateX(${-cameraX}px)">
      ...
      ${shelfPositions.map((x, index) => { const powered = (state!.shelfRooms[index] ?? 1) < activeRoomCount; const resonances = synergiesForShelf(index); const label = resonances.length ? ` · ${resonances.map(synergy => `${synergy.tag}×${synergy.count}`).join("+")}` : ""; return `<div class="world-shelf ${powered ? "powered" : "unpowered"} ${resonances.length ? "resonating" : ""}" style="left:${x}px" data-shelf="${index}"><div class="shelf-books"></div><span>책장 ${index + 1} · ${powered ? "활성" : "비활성"}${label}</span></div>`; }).join("")}
...
function bindSpatialActions(worldWidth: number, shelfPositions: number[]) {
  const viewport = app.querySelector<HTMLElement>(".house-viewport")!;
  const world = app.querySelector<HTMLElement>(".house-world")!;
  const character = app.querySelector<HTMLElement>(".character")!;
  const prompt = app.querySelector<HTMLElement>(".book-prompt")!;
  const hearthPrompt = app.querySelector<HTMLElement>(".hearth-prompt")!;
  const doorPrompt = app.querySelector<HTMLElement>(".door-prompt")!;
  const panel = app.querySelector<HTMLElement>(".room-panel")!;
  let startPointerX = 0;
  ...
  viewport.addEventListener("pointerdown", event => { if ((event.target as HTMLElement).closest(".room-zone")) return; startPointerX = event.clientX; startCamera = cameraX; dragged = false; viewport.setPointerCapture(event.pointerId); });
  viewport.addEventListener("pointermove", event => { if (!viewport.hasPointerCapture(event.pointerId)) return; const delta = event.clientX - startPointerX; if (Math.abs(delta) > 5) dragged = true; cameraX = startCamera - delta; paint(); });
  viewport.addEventListener("pointerup", event => { if (!dragged) { const rect = viewport.getBoundingClientRect(); characterX = Math.max(55, Math.min(worldWidth - 55, cameraX + event.clientX - rect.left)); character.classList.add("walking"); window.setTimeout(() => character.classList.remove("walking"), 550); paint(); } });
  app.querySelectorAll<HTMLElement>("[data-room]").forEach(room => room.addEventListener("click", () => { selectedRoom = Number(room.dataset.room); const count = state!.shelfRooms.filter(shelfRoom => shelfRoom === selectedRoom).length; app.querySelectorAll(".room-zone").forEach(el => el.classList.remove("selected")); room.classList.add("selected"); panel.classList.add("open"); panel.querySelector("h3")!.textContent = selectedRoom === 0 ? "화롯방" : selectedRoom === 1 ? "서고" : `빈 방 ${selectedRoom}`; panel.querySelector<HTMLElement>("h3 + p")!.textContent = selectedRoom === 0 ? "화로가 집 전체에 온기를 보냅니다." : `이 공간에는 책장이 ${count}/2개 배치되어 있습니다.`; const shelfButton = panel.querySelector<HTMLButtonElement>('[data-action="shelf"]')!; shelfButton.disabled = selectedRoom === 0 || count >= 2; shelfButton.querySelector("small")!.textContent = selectedRoom === 0 ? "서고에만 배치할 수 있습니다" : count >= 2 ? "이 방의 책장 한도에 도달했습니다" : `이 방 ${count}/2 · 수용량 +4`; }));
  app.querySelector(".close-room")?.addEventListener("click", () => panel.classList.remove("open"));
  app.querySelector('[data-action="menu"]')?.addEventListener("click", renderMenu);
  app.querySelector('[data-action="books"]')?.addEventListener("click", renderBooks);
  app.querySelector('[data-action="catalog"]')?.addEventListener("click", renderCatalog);
  app.querySelector('[data-action="prepare"]')?.addEventListener("click", renderExpeditionPrep);
  app.querySelector('[data-action="energy-flow"]')?.addEventListener("click", renderEnergyFlow);
  app.querySelector('[data-action="next"]')?.addEventListener("click", endDay);
  app.querySelector('[data-action="shelf"]')?.addEventListener("click", () => { if (!state) return; if (selectedRoom === 0) return toast("책장은 서고에만 놓을 수 있습니다."); if (state.shelfRooms.filter(room => room === selectedRoom).length >= 2) return toast("한 방에는 책장을 두 개까지만 놓을 수 있습니다."); const cost = 3 + state.shelves; if (state.firewood < cost) return toast(`땔감이 ${cost - state.firewood}만큼 부족합니다.`); state.firewood -= cost; state.shelves++; state.shelfRooms.push(selectedRoom); saveGame(); renderInteractiveHouse(); toast("선택한 서고에 새 책장을 놓았습니다."); });
  app.querySelector('[data-action="expand"]')?.addEventListener("click", () => { if (!state) return; const cost = 7 + state.houseLevel * 3; if (state.firewood < cost) return toast(`땔감이 ${cost - state.firewood}만큼 부족합니다.`); state.firewood -= cost; state.houseLevel++; saveGame(); renderInteractiveHouse(); toast("집 끝에 빈 방이 생겼습니다. 책장은 별도로 설치해야 합니다."); });
  window.onkeydown = event => { if (event.key.toLowerCase() === "e") { if (!doorPrompt.hidden && !(state!.crisis.warned && state!.day < state!.crisis.nextDay)) return renderExpeditionPrep(); if (!hearthPrompt.hidden) return renderEnergyFlow(); if (!prompt.hidden) return renderBooks(); } if (["a", "arrowleft", "d", "arrowright"].includes(event.key.toLowerCase())) { const direction = ["a", "arrowleft"].includes(event.key.toLowerCase()) ? -1 : 1; characterX = Math.max(55, Math.min(worldWidth - 55, characterX + direction * 45)); cameraX = characterX - viewport.clientWidth / 2; paint(); } };
  paint();
}
function bindHouseActions() {
  app.querySelector('[data-action="menu"]')?.addEventListener("click", renderMenu); app.querySelector('[data-action="books"]')?.addEventListener("click", renderBooks);
  app.querySelector('[data-action="shelf"]')?.addEventListener("click", () => { if (!state) return; const cost = 3 + state.shelves; if (state.firewood < cost) return toast(`땔감이 ${cost - state.firewood}만큼 부족합니다.`); state.firewood -= cost; state.shelves++; saveGame(); renderHouse(); toast("새 책장을 완성했습니다."); });
  app.querySelector('[data-action="expand"]')?.addEventListener("click", () => { if (!state) return; const cost = 7 + state.houseLevel * 3; if (state.firewood < cost) return toast(`땔감이 ${cost - state.firewood}만큼 부족합니다.`); state.firewood -= cost; state.houseLevel++; state.shelves++; saveGame(); renderHouse(); toast("집이 한층 넓어졌습니다."); }); app.querySelector('[data-action="next"]')?.addEventListener("click", endDay);
}
function endDay() {
  if (!state) return;
  ...
  if (!hadFood) state.warmth = Math.max(10, state.warmth - 8);
  ...
  saveGame();
  if (state.health <= 0) return renderGameOver(coldDamage ? "화로가 식은 밤, 저체온증이 마지막 온기를 빼앗았다." : "굶주림 속에서 더는 아침을 맞지 못했다.");
  renderHouse();
...
function ensureDiscoveryHistory() {
  if (!state) return;
  if (!Array.isArray(state.discoveredBookIds)) state.discoveredBookIds = [];
  state.books.forEach(book => { if (!state!.discoveredBookIds.includes(book.id)) state!.discoveredBookIds.push(book.id); });
}
...
function renderCatalog() {
  ...
  app.querySelector('[data-action="back"]')?.addEventListener("click", renderInteractiveHouse);
  app.querySelectorAll<HTMLButtonElement>("[data-catalog]").forEach(button => button.addEventListener("click", () => { selectedCatalogIndex = Number(button.dataset.catalog); renderCatalog(); }));
}
...
function renderEnergyFlow() {
  ...
  const center = 200;
  const radius = 118;
  const point = (index: number, distance: number) => { const angle = (-90 + index * 60) * Math.PI / 180; return `${center + Math.cos(angle) * distance},${center + Math.sin(angle) * distance}`; };
  ...
  const colors: Record<EmotionName, string> = { 희망: "#e0b866", 불안: "#82a9a6", 공포: "#816b9e", 슬픔: "#668aa8", 분노: "#b85f4d", 애착: "#a87978" };
...
function renderCrisisWarning() {
  ...
  app.querySelector('[data-action="acknowledge"]')?.addEventListener("click", renderInteractiveHouse);
}

function canMeetProduction(cost?: Partial<EmotionEnergy>) {
  if (!state || !cost) return true;
  const production = emotionProduction();
  return EMOTIONS.every(emotion => production[emotion] >= (cost[emotion] || 0));
}

function arbitraryProductionRequirement(cost?: Partial<EmotionEnergy>) {
  if (!cost) return 0;
  return EMOTIONS.reduce((sum, emotion) => sum + (cost[emotion] || 0), 0) * 2;
}

function canMeetArbitraryProduction(cost?: Partial<EmotionEnergy>) {
  const production = emotionProduction();
  const total = EMOTIONS.reduce((sum, emotion) => sum + production[emotion], 0);
  return total >= arbitraryProductionRequirement(cost);
}

function crisisTier(day: number) { return Math.max(1, Math.min(6, Math.round(day / 15))); }

function scaledCrisisCost(cost: Partial<EmotionEnergy> | undefined, day: number): Partial<EmotionEnergy> | undefined {
  if (!cost) return undefined;
  const tier = crisisTier(day);
  const multipliers = [.35, .5, .65, .8, 1, 1.25];
  const maxTypes = tier <= 2 ? 1 : tier <= 4 ? 2 : 6;
  const result: Partial<EmotionEnergy> = {};
...
function scaledCrisisEffect(effect: CrisisEffect | undefined, day: number): CrisisEffect {
  if (!effect) return {};
  const factors = [.45, .6, .75, .9, 1, 1.15];
  const factor = factors[crisisTier(day) - 1];
  const scale = (value?: number) => value === undefined ? undefined : value < 0 ? -Math.max(1, Math.ceil(Math.abs(value) * factor)) : value;
...
function specificCrisisAdvantage(effect: CrisisEffect, day: number): CrisisEffect {
  const tier = crisisTier(day);
  const advantaged = { ...effect };
  if ((advantaged.food || 0) > 0) advantaged.food = (advantaged.food || 0) + Math.max(1, Math.ceil(tier / 2));
  if ((advantaged.firewood || 0) > 0) advantaged.firewood = (advantaged.firewood || 0) + Math.max(1, Math.ceil(tier / 2));
  if ((advantaged.warmth || 0) > 0) advantaged.warmth = Math.min(45, (advantaged.warmth || 0) + 5 + tier);
  if (!advantaged.food && !advantaged.firewood && !advantaged.warmth && !advantaged.health) advantaged.warmth = 5 + tier;
  return advantaged;
}

function arbitraryCrisisEffect(day: number): CrisisEffect {
  if (Math.random() < .35) return {};
  const tier = crisisTier(day);
  return { health: -(2 + tier * 2), warmth: -(3 + tier * 2) };
}

function formatEmotionCost(cost?: Partial<EmotionEnergy>) {
  if (!cost) return "감정 생산 요구 없음";
  return EMOTIONS.filter(emotion => cost[emotion]).map(emotion => `${emotion} +${cost[emotion]}/일`).join(" · ");
}

function describeCrisisAftermath(effect: CrisisEffect, fatal: boolean) {
  if (fatal) return "사건이 잦아든 뒤에도 주인공은 다시 일어나지 못했다. 화로의 불빛은 한동안 빈 방을 비추다가, 기록의 마지막 문장과 함께 천천히 작아졌다.";
  const lines: string[] = [];
  if ((effect.health || 0) < 0) lines.push("몸에 남은 상처는 사건이 환상이 아니었음을 증명했다. 숨을 고를 때마다 그날의 냉기와 소리가 되살아났다.");
  if ((effect.food || 0) < 0 || (effect.firewood || 0) < 0) lines.push("창고에는 눈에 띄는 빈자리가 생겼다. 다음 며칠은 평소보다 훨씬 신중하게 배급해야 한다.");
  if ((effect.warmth || 0) < 0) lines.push("벽과 바닥에 밴 냉기는 화로를 다시 지핀 뒤에도 쉽게 물러나지 않았다.");
  if ((effect.food || 0) > 0 || (effect.firewood || 0) > 0) lines.push("위기가 남긴 것 중에는 쓸 만한 물자도 있었다. 뜻밖의 수확이 다음 밤을 버틸 작은 여유가 되었다.");
  if ((effect.warmth || 0) > 0) lines.push("되살아난 불꽃이 방마다 번지며 오래 얼어 있던 벽을 천천히 녹였다.");
  if (effect.loseBook) lines.push("서가에 생긴 빈칸은 다른 책을 옮겨도 감춰지지 않았다. 사라진 이야기가 있던 자리만 유난히 차갑다.");
...
function renderMidgameCrisis() {
  ...
  app.innerHTML = `<main class="crisis-screen active"><section><p class="eyebrow">MIDGAME CRISIS · 제 ${state.day}일 · 위기 강도 ${"Ⅰ".repeat(tier)}</p><span class="omen-mark">◆</span><h1>${crisis.title}</h1><p class="crisis-prose">${crisis.scene}</p><div class="emotion-tags">${crisis.emotions.map(emotion => `<span>${emotion}</span>`).join("")}</div><p class="lock-notice">현재 전체 감정 생산량 +${totalProduction}/일 · 저장량이 아닌 활성 책의 일일 생산량으로 대응합니다.</p><div class="crisis-choices">${crisis.choices.map((choice, index) => { const scaledCost = scaledCrisisCost(choice.cost, state!.crisis.nextDay); if (!scaledCost) return `<button data-crisis-choice="${index}" data-crisis-mode="none"><strong>${choice.label}</strong><p>${choice.description}</p><small>대응 실패 · 큰 피해 예상</small></button>`; const specificReady = canMeetProduction(scaledCost); const arbitraryRequired = arbitraryProductionRequirement(scaledCost); const arbitraryReady = canMeetArbitraryProduction(scaledCost); return `<button data-crisis-choice="${index}" data-crisis-mode="specific" ${specificReady ? "" : "disabled"}><strong>${choice.label}</strong><p>위기에 맞는 감정으로 완전히 제어해 추가 이득을 얻는다.</p><small>${formatEmotionCost(scaledCost)}${specificReady ? " · 유리한 결과" : " · 생산량 부족"}</small></button><button data-crisis-choice="${index}" data-crisis-mode="arbitrary" ${arbitraryReady ? "" : "disabled"}><strong>다른 감정을 끌어모은다</strong><p>위기를 무마하지만 감정 충돌로 작은 후유증이 생길 수 있다.</p><small>아무 감정 합계 +${arbitraryRequired}/일${arbitraryReady ? " · 피해 없음 또는 경미" : " · 현재 생산량 부족"}</small></button>`; }).join("")}</div></section></main>`;
  app.querySelectorAll<HTMLButtonElement>("[data-crisis-choice]").forEach(button => button.addEventListener("click", () => resolveMidgameCrisis(Number(button.dataset.crisisChoice), button.dataset.crisisMode || "none")));
}

function resolveMidgameCrisis(choiceIndex: number, mode: string) {
  if (!state) return;
  const crisis = MIDGAME_CRISES.find(entry => entry.id === state!.crisis.eventId) || MIDGAME_CRISES[0];
  ...
  if (mode === "specific" && !canMeetProduction(actualCost)) return toast("필요한 감정의 일일 생산량이 부족합니다.");
  if (mode === "arbitrary" && !canMeetArbitraryProduction(actualCost)) return toast("전체 감정 일일 생산량이 부족합니다.");
  ...
  let lostBook = "";
  if (effect.loseBook && state.books.length) { const lost = state.books.splice(Math.floor(Math.random() * state.books.length), 1)[0]; lostBook = ` ‘${lost.title}’의 기록을 영구적으로 잃었다.`; }
  else if (mode === "none" && choice.effect?.loseBook) lostBook = " 그러나 위기의 강도가 아직 낮았기에, 흩어진 문장 대부분을 아슬아슬하게 다시 묶어낼 수 있었다.";
  const resolvedDay = eventDay;
  const nextDay = resolvedDay + 15;
  const previousId = crisis.id;
  if (nextDay <= 90) { const candidates = MIDGAME_CRISES.filter(entry => entry.id !== previousId); state.crisis = { nextDay, eventId: candidates[Math.floor(Math.random() * candidates.length)].id, warned: false }; } else state.crisis = { nextDay: 105, eventId: "", warned: false };
  saveGame();
  ...
  app.querySelector('[data-action="continue"]')?.addEventListener("click", () => fatal ? renderGameOver(`${crisis.title} 사건이 피난처의 마지막 기록이 되었다.`) : renderInteractiveHouse());
}
...
function renderExpeditionPrep() {
  ...
  const toolSlots = app.querySelector<HTMLElement>(".tool-slots")!;
  toolSlots.innerHTML = state.tools.length ? state.tools.map(id => { const tool = toolById(id); if (!tool) return ""; const selected = prepToolIds.includes(id); return `<button class="tool-loadout ${selected ? "selected" : ""}" data-loadout-tool="${id}"><span>${tool.special ? "◆" : "+"}</span><strong>${tool.name}</strong><small>${tool.description}${tool.consumable ? " · 소모품" : ""}</small></button>`; }).join("") : `<div class="empty-tools"><strong>보유한 도구가 없습니다.</strong><small>탐사 중 폐허와 생존자의 흔적에서 발견할 수 있습니다.</small></div>`;
  app.querySelector('[data-action="back"]')?.addEventListener("click", renderInteractiveHouse);
  app.querySelectorAll<HTMLButtonElement>("[data-supply]").forEach(button => button.addEventListener("click", () => { const delta = Number(button.dataset.delta); if (button.dataset.supply === "food") prepFood = Math.max(0, Math.min(state!.food, prepFood + delta)); else prepWood = Math.max(0, Math.min(state!.firewood, prepWood + delta)); renderExpeditionPrep(); }));
  app.querySelectorAll<HTMLButtonElement>("[data-loadout-book]").forEach(button => button.addEventListener("click", () => { const id = button.dataset.loadoutBook!; if (prepBookIds.includes(id)) prepBookIds = prepBookIds.filter(entry => entry !== id); else if (prepBookIds.length < 2) prepBookIds.push(id); else return toast("책은 두 권까지만 가져갈 수 있습니다."); renderExpeditionPrep(); }));
  app.querySelectorAll<HTMLButtonElement>("[data-loadout-tool]").forEach(button => button.addEventListener("click", () => { const id = button.dataset.loadoutTool!; if (prepToolIds.includes(id)) prepToolIds = prepToolIds.filter(entry => entry !== id); else if (prepToolIds.length < 2) prepToolIds.push(id); else return toast("도구는 두 개까지만 챙길 수 있습니다."); renderExpeditionPrep(); }));
  app.querySelector('[data-action="depart"]')?.addEventListener("click", startExpedition);
}
...
function hasExpeditionTool(id: string) {
  if (!expedition) return false;
  if (id === "rope" && expedition.selectedToolIds.includes("silken-line")) return true;
  return expedition.selectedToolIds.includes(id) && !expedition.consumedToolIds.includes(id);
}

function consumeExpeditionTool(id: string) {
  if (!expedition) return;
  if (id === "rope" && expedition.selectedToolIds.includes("silken-line")) return;
  if (!expedition.consumedToolIds.includes(id)) expedition.consumedToolIds.push(id);
}

function createRouteOptions(): RouteOption[] {
  const kinds: RouteKind[] = ["safe", "ruins", "tracks", "signal"];
  for (let index = kinds.length - 1; index > 0; index--) { const swap = Math.floor(Math.random() * (index + 1)); [kinds[index], kinds[swap]] = [kinds[swap], kinds[index]]; }
  return kinds.slice(0, 3).map((kind, index) => { const variants = ROUTE_VARIANTS[kind]; const variant = variants[Math.floor(Math.random() * variants.length)]; const misleading = Math.random() < .12; const alternatives = kinds.filter(entry => entry !== kind); const actualKind = misleading ? alternatives[Math.floor(Math.random() * alternatives.length)] : kind; return { ...variant, id:`${expedition?.turn || 1}-${index}-${kind}`, kind, actualKind }; });
}

function weightedEvent(kind: RouteKind): EventKind {
  const tables: Record<RouteKind, Array<[EventKind, number]>> = {
  ...
  for (const [event, weight] of tables[kind]) { cursor += weight; if (roll <= cursor) return event; }
  return "none";
}

function chooseRoute(routeId: string) {
  if (!expedition) return;
  const route = expedition.routes?.find(entry => entry.id === routeId);
  if (!route) return;
  ...
  if (expedition.event === "animal") expedition.animal = (["토끼", "늑대", "곰"] as const)[Math.floor(Math.random() * 3)];
  if (expedition.event === "book") { const owned = new Set([...(state?.books || []), ...expedition.lootBooks].map(book => book.id)); const locked = CATALOG_TITLES.map((_, index) => catalogBook(index)).filter(book => !owned.has(book.id)); expedition.discovery = locked[Math.floor(Math.random() * locked.length)]; if (!expedition.discovery) expedition.event = "resource"; else { const order = [0,1,2]; for (let index = order.length - 1; index > 0; index--) { const swap = Math.floor(Math.random() * (index + 1)); [order[index], order[swap]] = [order[swap], order[index]]; } expedition.bookChoiceOrder = order; } }
  if (expedition.event === "story") { const pool = ROUTE_STORY_POOLS[route.actualKind]; expedition.storyId = pool[Math.floor(Math.random() * pool.length)]; }
  renderExpedition();
}

function beginTurn() {
  if (!expedition) return;
  if (expedition.hp <= 0 && hasExpeditionTool("golden-feather")) { consumeExpeditionTool("golden-feather"); expedition.hp = 18; expedition.resultText = "황금빛 깃털이 부서지며 눈 위에 빛을 흩뿌렸다. 의식을 잃기 직전, 몸을 일으킬 마지막 힘이 돌아왔다."; }
  if (expedition.hp <= 0) { if (state) { state.health = 0; saveGame(); } return renderGameOver("눈보라 속에서 입은 상처를 끝내 견디지 못했다."); }
  if (expedition.turn > expedition.totalTurns) return renderSettlement();
...
function renderExpedition() {
  ...
  const eventCopy: Record<EventKind, [string, string]> = { none: ["아무 일도 일어나지 않았다", "한 턴 동안 눈 덮인 길을 돌아다녔지만 특별한 흔적도, 마주친 존재도 없었다. 시간과 보급만 조용히 줄어든다."], avalanche: ["산이 울리기 시작했다", "머리 위 설벽에 금이 번진다. 잠시 뒤 눈과 얼음이 굉음을 내며 비탈을 덮친다."], animal: [`${expedition.animal}와 마주쳤다`, expedition.animal === "토끼" ? "눈더미 사이에서 굶주린 토끼가 몸을 낮춘 채 도망칠 길을 찾고 있다." : expedition.animal === "늑대" ? "앙상한 늑대가 낮게 으르렁댄다. 갈비뼈가 드러날 만큼 굶주렸지만 눈빛은 날카롭다." : "거대한 곰이 얼어붙은 사체 옆에서 몸을 일으킨다. 흰 입김이 눈보라보다 거칠다."], resource: ["검게 드러난 잔해", "무너진 건물 틈에서 아직 쓸 수 있을 법한 목재 더미를 발견했다. 지붕은 위태롭게 기울어져 있다."], book: [expedition.discovery?.title || "눈 속의 문장", puzzle?.scene || "눈 속에서 정체를 알 수 없는 책 한 권을 발견했다."], story: [story?.title || "눈 속의 흔적", story?.scene || "낯선 흔적이 길을 가로막는다."] };
  ...
  const returnButton = app.querySelector<HTMLButtonElement>('.expedition-loot [data-choice="return"]');
  app.querySelector(".event-index")?.insertAdjacentHTML("beforebegin", `<p class="route-origin">선택한 경로 · ${expedition.chosenRoute?.title || "알 수 없는 길"}</p>`);
  returnButton?.insertAdjacentHTML("beforebegin", `<div class="tool-loot"><span>도구</span><strong>${expedition.lootToolIds.length}</strong><small>${expedition.selectedToolIds.map(id => `${toolById(id)?.name || id}${expedition!.consumedToolIds.includes(id) ? "(소모)" : ""}`).join(" · ") || "미장착"}</small></div>`);
  const eventChoices = app.querySelector<HTMLElement>(".event-choices");
  if (decision && eventChoices && expedition.hp < 100 && hasExpeditionTool("medkit")) eventChoices.insertAdjacentHTML("beforeend", `<button data-choice="use-medkit"><strong>응급처치를 한다</strong><small>응급처치 가방 소모 · 체력 25 회복</small></button>`);
  if (decision && eventChoices && expedition.event === "animal" && hasExpeditionTool("flare")) eventChoices.insertAdjacentHTML("beforeend", `<button data-choice="animal-flare"><strong>신호탄을 발사한다</strong><small>신호탄 소모 · 동물을 확실히 쫓아낸다</small></button>`);
  if (decision && eventChoices && expedition.event === "animal" && hasExpeditionTool("trap")) eventChoices.insertAdjacentHTML("beforeend", `<button data-choice="animal-trap"><strong>사냥 덫으로 유인한다</strong><small>사냥 덫 소모 · 피해 없이 식량 획득</small></button>`);
  if (decision && eventChoices && expedition.event === "avalanche" && hasExpeditionTool("rope")) eventChoices.insertAdjacentHTML("beforeend", `<button data-choice="avalanche-rope"><strong>몸을 밧줄로 고정한다</strong><small>${hasExpeditionTool("silken-line") ? "금빛 머리끈 사용" : "등반용 밧줄 소모"} · 피해 경감</small></button>`);
  app.querySelectorAll<HTMLButtonElement>("[data-choice]").forEach(button => button.addEventListener("click", () => handleExpeditionChoice(button.dataset.choice!)));
}

function renderEncounterAction() {
  if (!expedition) return "";
  if (expedition.event === "none") return `<button class="wide-action" data-choice="next">계속 걷는다 <span>→</span></button>`;
  return `<button class="wide-action" data-choice="observe">상황을 살핀다 <span>→</span></button>`;
}

function renderDecisionActions() {
  if (!expedition) return "";
  if (expedition.event === "avalanche") return `<div class="event-choices"><button data-choice="brace"><strong>몸을 웅크린다</strong><small>충격에 대비한다</small></button><button data-choice="run"><strong>바위 뒤로 달린다</strong><small>눈사태 범위에서 벗어나 본다</small></button></div>`;
  if (expedition.event === "animal") return `<div class="event-choices"><button data-choice="fight"><strong>싸운다</strong><small>책의 힘으로 맞선다 · 전투는 임시 판정</small></button><button data-choice="flee"><strong>도망간다</strong><small>동물에 따라 추격 확률이 달라진다</small></button></div>`;
  if (expedition.event === "resource") return `<div class="event-choices"><button data-choice="search"><strong>잔해 안으로 들어간다</strong><small>쓸 만한 땔감을 찾아본다</small></button><button data-choice="leave"><strong>그대로 지나친다</strong><small>위험해 보이는 구조물을 피한다</small></button></div>`;
  if (expedition.event === "story") { const storyId = expedition.storyId; const story = EXPEDITION_STORIES.find(entry => entry.id === storyId); const choices = [...(story?.choices || []), ...(TOOL_STORY_CHOICES[storyId || ""] || [])]; return `<div class="event-choices">${choices.map((entry, index) => { const available = !entry.requiredTool || hasExpeditionTool(entry.requiredTool); const tool = entry.requiredTool ? toolById(entry.requiredTool) : undefined; return `<button data-choice="story-${index}" ${available ? "" : "disabled"}><strong>${entry.label}</strong><small>${available ? entry.hint : `${tool?.name || "필요한 도구"}를 챙기지 않음`}</small></button>`; }).join("")}</div>`; }
  const hasBookSpace = state ? state.books.length + expedition.lootBooks.length < state.shelves * 4 : false;
  if (!hasBookSpace) return `<div class="event-choices book-choices no-space"><button data-choice="book-leave"><strong>책을 두고 지나간다</strong><small>남은 책장 공간이 없어 새로운 기록을 보관할 수 없다</small></button></div>`;
  const puzzle = expedition.discovery ? BOOK_PUZZLES[expedition.discovery.title] : undefined;
  const originalChoices = puzzle?.choices || ["책에 손을 댄다", "기다린다", "돌아선다"];
...
function handleExpeditionChoice(choice: string) {
  if (!expedition) return;
  if (choice === "return") return renderSettlement();
  if (choice === "next") { expedition.turn++; return beginTurn(); }
  if (choice === "observe") { expedition.phase = "decision"; return renderExpedition(); }
  let text = "";
  const toolChoice = ["use-medkit", "animal-flare", "animal-trap", "avalanche-rope"].includes(choice);
  if (choice === "use-medkit") { consumeExpeditionTool("medkit"); expedition.hp = Math.min(100, expedition.hp + 25); text = "바람을 막을 곳을 찾아 상처를 소독하고 압박 붕대를 감았다. 체력이 25 회복되었다."; }
  else if (choice === "animal-flare") { consumeExpeditionTool("flare"); text = `붉은 불꽃과 폭음에 ${expedition.animal}가 몸을 돌려 달아났다.`; }
  else if (choice === "animal-trap") { consumeExpeditionTool("trap"); const food = expedition.animal === "토끼" ? 1 : expedition.animal === "늑대" ? 2 : 4; expedition.lootFood += food; text = `접근로에 덫을 설치해 직접 싸우지 않고 ${expedition.animal}를 제압했다. 식량 ${food}개를 얻었다.`; }
  else if (choice === "avalanche-rope") { consumeExpeditionTool("rope"); const damage = 2 + Math.floor(Math.random() * 4); expedition.hp = Math.max(0, expedition.hp - damage); text = `몸을 단단히 고정해 눈사태에 휩쓸리는 것을 피했다. 체력이 ${damage} 감소했다.`; }
  if (toolChoice) { expedition.phase = "result"; expedition.resultText = text; return renderExpedition(); }
  if (expedition.event === "avalanche") { let damage = choice === "brace" ? 5 + Math.floor(Math.random() * 7) : 3 + Math.floor(Math.random() * 15); if (hasExpeditionTool("ember-lantern")) damage = Math.max(1, Math.ceil(damage / 2)); expedition.hp = Math.max(0, expedition.hp - damage); text = choice === "brace" ? `눈더미가 등을 짓눌렀지만 자세를 지켰다. 체력이 ${damage} 감소했다.${hasExpeditionTool("ember-lantern") ? " 등불이 빈 공간을 비춰 질식을 피했다." : ""}` : `비탈을 달렸으나 얼음 조각이 옆구리를 때렸다. 체력이 ${damage} 감소했다.`; }
  if (expedition.event === "resource") { if (choice === "search") { const found = 1 + Math.floor(Math.random() * 3) + (hasExpeditionTool("giant-belt") ? 2 : 0); expedition.lootWood += found; text = `기울어진 지붕 아래에서 마른 목재 ${found}개를 꺼냈다.${hasExpeditionTool("giant-belt") ? " 허리띠가 무거운 목재의 무게를 받아냈다." : ""}`; } else text = "무너지는 소리를 등 뒤로 남기고 길을 재촉했다. 얻은 것도, 잃은 것도 없다."; }
  if (expedition.event === "animal") { const animal = expedition.animal!; const food = animal === "토끼" ? 1 : animal === "늑대" ? 2 : 4; const fightChance = (animal === "토끼" ? .9 : animal === "늑대" ? .62 : .38) + expedition.selectedBookIds.length * .12 + (hasExpeditionTool("hunter-knife") ? .25 : 0); const fleeChance = animal === "토끼" ? .95 : animal === "늑대" ? .55 : .3; if (choice === "flee" && Math.random() < fleeChance) text = `${animal}의 시야에서 벗어났다. 한동안 발자국을 지우며 우회했다.`; else { const won = Math.random() < fightChance; if (won) { expedition.lootFood += food; text = choice === "flee" ? `도망치던 중 따라잡혀 싸울 수밖에 없었다. 간신히 승리해 식량 ${food}개를 얻었다.` : `책과 장비를 이용해 ${animal}를 쓰러뜨리고 식량 ${food}개를 얻었다.`; } else { const damage = animal === "토끼" ? 4 : animal === "늑대" ? 16 : 28; expedition.hp = Math.max(0, expedition.hp - damage); text = `${animal}를 막아내지 못하고 물러났다. 체력이 ${damage} 감소했다. 전투 시스템은 추후 상세 구현된다.`; } } }
  if (expedition.event === "story") { const storyId = expedition.storyId; const story = EXPEDITION_STORIES.find(entry => entry.id === storyId); const choices = [...(story?.choices || []), ...(TOOL_STORY_CHOICES[storyId || ""] || [])]; const picked = choices[Number(choice.split("-")[1])]; if (picked && (!picked.requiredTool || hasExpeditionTool(picked.requiredTool))) { const success = picked.chance === undefined || Math.random() < picked.chance; const food = success ? picked.food || 0 : picked.failureFood || 0; const wood = success ? picked.wood || 0 : picked.failureWood || 0; const health = success ? picked.health || 0 : picked.failureHealth || 0; expedition.lootFood = Math.max(0, expedition.lootFood + food); expedition.lootWood = Math.max(0, expedition.lootWood + wood); expedition.hp = Math.max(0, Math.min(100, expedition.hp + health)); if (success && picked.consumeTool && picked.requiredTool) consumeExpeditionTool(picked.requiredTool); if (success && picked.grantTool && !expedition.lootToolIds.includes(picked.grantTool) && !state?.tools.includes(picked.grantTool)) expedition.lootToolIds.push(picked.grantTool); text = success ? picked.result : picked.failure || "선택은 기대한 결과로 이어지지 않았다."; } }
  if (expedition.event === "book") { if (choice === "book-leave") text = "가져갈 자리가 없다는 것을 알면서도 한동안 책 앞을 떠나지 못했다. 결국 눈 위의 이야기를 뒤로하고 발걸음을 옮겼다."; else { const displayedIndex = Number(choice.split("-")[1]); const picked = (expedition.bookChoiceOrder || [0,1,2])[displayedIndex]; const puzzle = BOOK_PUZZLES[expedition.discovery!.title]; if (picked === puzzle.correct) { expedition.lootBooks.push(expedition.discovery!); text = `${puzzle.success} ‘${expedition.discovery!.title}’을 획득했다.`; } else text = puzzle.failure; } }
...
function renderSettlement() {
  if (!expedition || !state) return;
  if (expedition.hp <= 0) { state.health = 0; saveGame(); return renderGameOver("탐사에서 입은 치명상을 이겨내지 못했다."); }
  ...
  expedition.lootToolIds.forEach(id => { if (!state!.tools.includes(id)) state!.tools.push(id); });
  const newBooks: Book[] = [];
  keptBooks.filter(book => !state!.books.some(owned => owned.id === book.id)).forEach(book => { const shelfId = firstAvailableShelf(newBooks); if (shelfId >= 0) { book.shelfId = shelfId; newBooks.push(book); } });
  ensureDiscoveryHistory();
  newBooks.forEach(book => { if (!state!.discoveredBookIds.includes(book.id)) state!.discoveredBookIds.push(book.id); });
  ...
  if (expedition.lootToolIds.length) app.querySelector(".settlement-loot")?.insertAdjacentHTML("afterend", `<div class="found-tools"><span>새로운 도구</span>${expedition.lootToolIds.map(id => `<strong>${toolById(id)?.name || id}</strong>`).join("")}</div>`);
  app.querySelector('[data-action="home"]')?.addEventListener("click", () => { expedition = null; prepBookIds = []; prepToolIds = []; renderInteractiveHouse(); });
}

function renderGameOver(reason: string) {
  window.onkeydown = null;
  app.innerHTML = `<main class="game-over-screen"><section><p class="eyebrow">THE LAST RECORD</p><div class="dead-flame"></div><h1>이야기가 끝났습니다</h1><p>${reason}</p><div class="final-record"><span>마지막 생존 기록</span><strong>제 ${state?.day || 1}일</strong><small>보존한 책 ${state?.books.length || 0}권</small></div><button data-action="restart">새로운 기록을 시작한다</button><button class="quiet" data-action="menu">메인 메뉴로</button></section></main>`;
  app.querySelector('[data-action="restart"]')?.addEventListener("click", () => { state = initialState(); expedition = null; prepFood = 1; prepWood = 1; prepBookIds = []; prepToolIds = []; saveGame(); renderInteractiveHouse(); });
  app.querySelector('[data-action="menu"]')?.addEventListener("click", renderMenu);
}

function bookManagementCard(book: Book, reach: number, compact = false) {
  if (!state) return "";
  const powered = bookIsPowered(book);
  const shelfId = book.shelfId ?? 0;
  ...
  const placementOptions = state.shelfRooms.map((_, index) => {
    const shelfBooks = state!.books.filter(entry => (entry.shelfId ?? 0) === index);
    const status = `${state!.shelfRooms[index] < reach ? "활성" : "단절"}`;
    if (index === shelfId) return `<option value="stay" selected>${shelfLabel(index)} · ${shelfBooks.length}/4 · ${status}</option>`;
    if (shelfBooks.length < 4) return `<option value="move:${index}">${shelfLabel(index)} · ${shelfBooks.length}/4 · ${status}</option>`;
...
function renderBooks() {
  ...
  app.querySelector('[data-action="back"]')?.addEventListener("click", renderHouse);
  app.querySelectorAll<HTMLButtonElement>("[data-books-view]").forEach(button => button.addEventListener("click", () => { booksView = button.dataset.booksView as "shelves" | "all"; renderBooks(); }));
  app.querySelectorAll<HTMLButtonElement>("[data-shelf-view]").forEach(button => button.addEventListener("click", () => { selectedBookShelfId = Number(button.dataset.shelfView); renderBooks(); }));
  app.querySelectorAll<HTMLButtonElement>("[data-book]").forEach(button => button.addEventListener("click", () => { if (!state) return; const book = state.books.find(entry => entry.id === button.dataset.book); if (!book) return; book.contained = !book.contained; saveGame(); renderBooks(); }));
  app.querySelectorAll<HTMLSelectElement>("[data-move-book]").forEach(select => select.addEventListener("change", () => {
    if (!state || select.value === "stay") return;
    const book = state.books.find(entry => entry.id === select.dataset.moveBook);
    if (!book) return;
    const sourceShelf = book.shelfId ?? 0;
    const [action, value] = select.value.split(":");
    if (action === "move") {
      const destination = Number(value);
      const used = state.books.filter(entry => (entry.shelfId ?? 0) === destination).length;
      if (used >= 4) { toast("그 사이 선택한 책장이 가득 찼습니다."); return renderBooks(); }
      book.shelfId = destination;
      saveGame(); renderBooks(); toast(`${shelfLabel(destination)}으로 책을 옮겼습니다.`);
      return;
    }
    if (action === "swap") {
      const target = state.books.find(entry => entry.id === value);
      if (!target || target.id === book.id) return renderBooks();
      const destination = target.shelfId ?? 0;
      book.shelfId = destination;
      target.shelfId = sourceShelf;
      saveGame(); renderBooks();
      toast(`‘${target.title}’과 자리를 교환했습니다.`);
    }
  }));
  app.querySelectorAll<HTMLButtonElement>("[data-burn-book]").forEach(button => button.addEventListener("click", () => burnBook(button.dataset.burnBook!)));
}

function burnBook(bookId: string) {
  if (!state) return;
  ensureDiscoveryHistory();
  const book = state.books.find(entry => entry.id === bookId);
  if (!book || !confirm(`‘${book.title}’을 화로에 넣을까요? 원본 책은 영구적으로 사라지지만 도감의 발견 기록은 남습니다.`)) return;
  ...
  if (rewardId && !state.tools.includes(rewardId)) { state.tools.push(rewardId); const tool = toolById(rewardId); message = `책장이 모두 재가 된 뒤에도 ${tool?.name || "정체불명의 장비"}만은 불 속에 남았습니다.`; }
  saveGame();
  renderBooks();
  toast(message);
}
